...
def show_status_legacy(conn=None):
    ...
    baselines = conn.execute("SELECT * FROM baselines ORDER BY created_at DESC").fetchall()
    changes = conn.execute("SELECT * FROM system_changes ORDER BY created_at DESC").fetchall()
