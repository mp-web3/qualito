...
All functions accept a conn parameter (sqlite3.Connection or SA Connection).
...
def compute_workspace_baselines(
    ...
    *,
    ...
    if isinstance(conn, sqlite3.Connection):
        return compute_workspace_baselines_legacy(
            conn, workspace,
...
def check_consecutive_failures(
    ...
    *,
    ...
    if isinstance(conn, sqlite3.Connection):
        return check_consecutive_failures_legacy(
            conn, run_id, workspace,
...
def check_dqi_burn_rate(
    ...
    *,
    ...
    if isinstance(conn, sqlite3.Connection):
        return check_dqi_burn_rate_legacy(
            conn, run_id, workspace,
            fast_window=fast_window, slo_quality=slo_quality,
            slo_cost=slo_cost, baseline_window=baseline_window,
        )
...
def check_cost_anomaly(
    ...
    *,
    ...
    if isinstance(conn, sqlite3.Connection):
        return check_cost_anomaly_legacy(
            conn, run_id, workspace,
            fast_window=fast_window, slo_quality=slo_quality,
            slo_cost=slo_cost, baseline_window=baseline_window,
        )
...
def check_error_pattern_spike(
    ...
    if isinstance(conn, sqlite3.Connection):
        return check_error_pattern_spike_legacy(conn, run_id, workspace)
...
def check_run(conn, run_id: str) -> list[dict]:
    ...
    """
    if isinstance(conn, sqlite3.Connection):
        return check_run_legacy(conn, run_id)
...
def check_monitoring_close(
    ...
    *,
    ...
    if isinstance(conn, sqlite3.Connection):
        return check_monitoring_close_legacy(
            conn,
            monitoring_clean_threshold=monitoring_clean_threshold,
...
def check_auto_resolve(
    ...
    *,
    ...
    """
    if isinstance(conn, sqlite3.Connection):
        return check_auto_resolve_legacy(
            conn,
...
def compute_workspace_baselines_legacy(
    ...
    *,
    ...
    """Legacy: Compute quality baselines using raw SQL."""
    total_runs = conn.execute(
...
def check_consecutive_failures_legacy(
    conn: sqlite3.Connection,
    run_id: str,
    workspace: str,
    *,
    ...
    """Legacy: Detect consecutive failures using raw SQL."""
    rows = conn.execute("""
...
def check_dqi_burn_rate_legacy(
    conn: sqlite3.Connection,
    run_id: str,
    workspace: str,
    *,
    ...
    """Legacy: Detect DQI burn rate using raw SQL."""
    baselines = compute_workspace_baselines(
        conn, workspace,
        slo_quality=slo_quality, slo_cost=slo_cost,
        baseline_window=baseline_window,
...
def check_cost_anomaly_legacy(
    ...
    *,
    ...
    """Legacy: Detect cost anomalies using raw SQL."""
    baselines = compute_workspace_baselines(
        conn, workspace,
        slo_quality=slo_quality, slo_cost=slo_cost,
        baseline_window=baseline_window,
...
def check_error_pattern_spike_legacy(
    conn: sqlite3.Connection,
    run_id: str,
    workspace: str,
) -> dict | None:
    """Legacy: Detect error pattern spikes using raw SQL."""
    recent_runs = conn.execute("""
        SELECT id FROM runs
        WHERE workspace = ?
        ORDER BY started_at DESC
...
def check_run_legacy(conn: sqlite3.Connection, run_id: str) -> list[dict]:
    """Legacy: Run all checks using raw SQL."""
    row = conn.execute(
...
def check_monitoring_close_legacy(
    conn: sqlite3.Connection,
    *,
    monitoring_clean_threshold: int = DEFAULT_MONITORING_CLEAN_THRESHOLD,
    slo_quality: float = DEFAULT_SLO_QUALITY,
) -> list[dict]:
    """Legacy: Auto-close monitoring incidents using raw SQL."""
    monitoring_incidents = conn.execute(
...
def check_auto_resolve_legacy(
    conn: sqlite3.Connection,
    *,
    fast_window: int = DEFAULT_FAST_WINDOW,
    slo_quality: float = DEFAULT_SLO_QUALITY,
) -> list[dict]:
    """Legacy: Auto-resolve incidents using raw SQL."""
    open_incidents = conn.execute(
